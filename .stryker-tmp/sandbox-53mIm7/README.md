...
2) Mutation Testing (Stryker)
  - Instalar já configurado (`stryker.conf.json`).
  - Rodar: `npm run mutate`.
  - Interprete: se mutantes sobrevivem em regra de negócio, adicione caso de teste que mate a mutação.
  - Desative mutation em arquivos utilitários triviais para focar no domínio.
...
4) Tipos Avançados
  - Introduzir `Money` (interno representado em centavos) para eliminar arredondamentos repetidos.
  - Usar `readonly` em campos de entidades para reforçar imutabilidade.
...
9) CI/CD
  - Pipeline: typecheck → lint → test → coverage threshold → mutation (subset) → build artefato.
  - Estratégia: rodar mutation em modo rápido (subset) em cada PR e completo 1x/dia.
